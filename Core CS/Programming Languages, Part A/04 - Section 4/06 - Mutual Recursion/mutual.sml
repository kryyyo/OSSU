fun match xs = (* example only, [1,2,1,2,1,2] *)
...
(* using higher-order function if there are no mutual recursion support *)
(* not as efficient as mutual recursion support *)
fun no_zeros_or_empty_string_t1_alternate (f, x) =
    case x of
	Foo i => i <> 0
      | Bar y => f y

fun no_zeroes_or_empty_string_t2_alternate x =
    case x of
	Baz s => size s > 0
