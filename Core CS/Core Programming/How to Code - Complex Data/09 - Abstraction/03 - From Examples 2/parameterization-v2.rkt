...
(check-expect (contains? "" (cons "UBC" (cons "McGill" empty))) false)
...
;; produce (list fn(n0) fn(n1) ...) given fn and (list n0 n1 ...)

(check-expect (map2 sqr empty) empty)
(check-expect (map2 sqr (list 3 4)) (list 9 16))
(check-expect (map2 sqrt (list 9 16)) (list 3 4))
(check-expect (map2 sub1 (list 9 16)) (list 8 15))
...
;; produces the list of all elements from a given list that satisfies the given predicate

(check-expect (filter2 string? empty) empty)
(check-expect (filter2 string? (list 1 -2 3 -4)) empty)
